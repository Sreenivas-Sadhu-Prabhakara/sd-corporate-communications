...
| POST | `/v1/communication-campaign-operating-session/initiate` | Initiate |
| GET | `/v1/communication-campaign-operating-session` | Retrieve (list) |
| GET | `/v1/communication-campaign-operating-session/{crId}/retrieve` | Retrieve |
| PUT | `/v1/communication-campaign-operating-session/{crId}/update` | Update |
| PUT | `/v1/communication-campaign-operating-session/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/communication-campaign-operating-session/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-corporate-communications:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-corporate-communications ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-corporate-communications` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
